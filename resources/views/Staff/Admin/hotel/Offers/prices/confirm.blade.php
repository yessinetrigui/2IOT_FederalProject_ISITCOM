@extends('Staff.Admin.Layouts.app')
@section("title")
hotel Add
@endsection
@section('content')

@if ($para == "Delete")
<form action="{{route('admin.hotel.manage.offers.doDelete', ['idHotel'=>$idHotel, 'id'=>$id])}}" method="post">
    @else
<form action="{{route('admin.hotel.manage.offers.doStop', ['idHotel'=>$idHotel, 'id'=>$id])}}" method="post">
    @endif
    @if ($para == "Delete")
    <h1>Confirm Delete for Offer ID: {{$id}}?</h1>
    @else
    <h1>Confirm {{$para}} for Offer  ID: {{$id}}?</h1>
    @endif
    @csrf
    <input class="font-xl  text-white font-medium bg-red p-4 my-2" style="background: #880808" value="Confirm" type="submit">
</form>
@endsection
